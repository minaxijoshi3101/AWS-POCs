...
a=$(aws ec2 create-image --instance-id i-02d52ed6507e66813 --name "EC2-ASP-time" --no-reboot --output json 2>&1)
...
if [[ $a == *"InvalidAMIName.Duplicate"* ]]; then
    ...
    b=$(aws ec2 create-image --instance-id i-02d52ed6507e66813 --name "EC2-ASP-time" --no-reboot --output json | jq -r .ImageId)
